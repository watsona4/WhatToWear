...
class ClothingSelector {
  constructor() {
    ...
    this.shortShirts = ["black", "grey", "maroon", "navy", "nnl"];

    this.sweaters = ["black", "grey", "maroon", "navy", "nnl"];

    this.coats = ["navy", "grey", "brown", "charcoal", "tan"];

    this.pantss = ["khaki", "tan", "grey", "jeans"];

    this.summerPants = [];
    this.summerCoats = ["tan"];

    // Key is coat, value is list of pants that go with coat.
    this.coatData = new Map(
      Object.entries({
        navy: ["grey", "tan", "khaki", "brown", "white"],
        tan: ["charcoal", "grey", "olive", "brown", "white"],
        brown: ["charcoal", "grey", "olive", "white"],
        olive: ["charcoal", "grey", "tan", "khaki", "brown"],
        grey: ["charcoal", "navy", "white"],
        blue: ["navy", "tan", "khaki", "white"],
        charcoal: ["grey", "khaki"],
      }),
    );

    // Key is shirt, value is list of sweaters that *dont* go with shirt.
    this.shirtData = new Map(
      Object.entries({
        maroon: ["maroon", "nnl"],
        green: ["nnl"],
        grey: ["grey", "navy", "nnl"],
    ...
    this.sweaterData = new Map(
      Object.entries({ black: ["khaki"], grey: ["grey"], navy: ["khaki"] }),
    );

    // Key is long shirt, value is list of pants that *dont* go with shirt.
    this.shirtPantData = new Map(
      Object.entries({ maroon: ["khaki"], green: ["grey"], grey: ["khaki"] }),
    );

    // Key is short shirt, value is list of pants that *dont* go with shirt.
    this.shortShirtPantData = new Map(
      Object.entries({ black: ["khaki"], grey: ["grey"], navy: ["khaki"] }),
    );

    // Key is sweater, value is list of coats that *dont* go with sweater.
    this.sweaterCoatData = new Map(
      Object.entries({ navy: ["navy"], khaki: ["tan"], charcoal: ["grey"] }),
    );
  }
  ...
  async loadData() {
    ...
    if (this.oldLongShirts == null) {
      this.oldLongShirts = [];
      this.longShirt = null;
    } else this.longShirt = this.oldLongShirts[this.oldLongShirts.length - 1];
    ...
    if (this.oldSweaters == null) {
      this.oldSweaters = [];
      this.sweater = null;
    } else this.sweater = this.oldSweaters[this.oldSweaters.length - 1];
  ...
  sweaterFilter(shirt) {
    ...
    let isFriday = (e) => (today.getDay() == 4) == (e == "nnl");
    let hasShirt = (elem) =>
      shirt in this.shirtData && this.shirtData.get(shirt).includes(elem);
    return (elem) =>
      !this.oldSweaters.includes(elem) && !hasShirt(elem) && isFriday(elem);
  }

  coatFilter(sweater) {
    let hasSweater = (elem) =>
      sweater in this.sweaterCoatData &&
      this.sweaterCoatData.get(sweater).includes(elem);
  ...
  pantsFilter(coat, sweater, longShirt, shortShirt) {
    ...
    let hasCoat = (e) =>
      typeof coat !== "undefined"
        ? coat in this.coatData && this.coatData.get(coat).includes(e)
        : true;
    let hasSweater = (e) =>
      typeof sweater !== "undefined"
        ? sweater in this.sweaterData &&
          this.sweaterData.get(sweater).includes(e)
        : false;
    let hasLongShirt = (e) =>
      typeof longShirt !== "undefined"
        ? longShirt in this.shirtPantData &&
          this.shirtPantData.get(longShirt).includes(e)
        : false;
    let hasShortShirt = (e) =>
      typeof shortShirt !== "undefined"
        ? shortShirt in this.shortShirtPantData &&
          this.shortShirtPantData.get(shortShirt).includes(e)
        : false;
    if (coat && sweater && longShirt) {
      ...
      return (elem) =>
        isFriday(elem) && !hasSweater(elem) && !hasLongShirt(elem);
    } else if (sweater && shortShirt) {
      return (elem) =>
        isFriday(elem) && !hasSweater(elem) && !hasShortShirt(elem);
    } else if (longShirt) {
  ...
  nextCombo(clothing) {
    ...
    if (
      ...
      for (const longShirt of this.longShirts.filter(this.longShirtFilter()))
        for (const sweater of this.sweaters.filter(
          this.sweaterFilter(longShirt),
        ))
          for (const coat of this.coats.filter(this.coatFilter(sweater)))
            for (const pants of this.pantss.filter(
              this.pantsFilter(coat, sweater, longShirt, undefined),
            ))
      ...
      for (const longShirt of this.longShirts.filter(this.longShirtFilter()))
        for (const sweater of this.sweaters.filter(
          ...
          for (const pants of this.pantss.filter(
            this.pantsFilter(undefined, sweater, longShirt, undefined),
          ))
      ...
      for (const longShirt of this.shortShirts.filter(this.shortShirtFilter()))
        for (const coat of this.coats.filter(this.coatFilter(sweater)))
          for (const pants of this.pantss.filter(
            this.pantsFilter(coat, undefined, longShirt, undefined),
          ))
      ...
      for (const longShirt of this.shortShirts.filter(this.shortShirtFilter()))
        for (const pants of this.pantss.filter(
          this.pantsFilter(undefined, undefined, longShirt, undefined),
        ))
      ...
      for (const shortShirt of this.shortShirts.filter(this.shortShirtFilter()))
        for (const coat of this.coats.filter(this.coatFilter(sweater)))
          for (const pants of this.pantss.filter(
            this.pantsFilter(coat, undefined, undefined, shortShirt),
          ))
      ...
      for (const shortShirt of this.shortShirts.filter(this.shortShirtFilter()))
        for (const pants of this.pantss.filter(
          this.pantsFilter(undefined, undefined, undefined, shortShirt),
        ))
  ...
  saveData() {
    ...
    else shortArray = this.oldShortShirts.slice(1, shortShirtNum);
    ...
    else longArray = this.oldLongShirts.slice(1, longShirtNum);
    ...
    else sweaterArray = this.oldSweaters.slice(1, sweaterNum);
