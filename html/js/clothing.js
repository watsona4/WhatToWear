import {
  ...
  saveData,
  loadData,
} from "./modules/clothing_selector.js";
...
function convertTemp(units, temp) {
  if (units == "wmoUnit:degC") return (temp * 9) / 5 + 32.0;
  return temp;
}
...
function getWeatherData(url) {
  $.getJSON(url, (data) => {
    ...
    $("#temp")
      ...
      .html("Temp: " + temperature.toFixed(1) + " &deg;F");
    ...
    $("#image").attr(
      "src",
      "js/icons/" + icons[current.weather_code][day_night].icon,
    );
...
function getClothing() {
  ...
  if (activity == "Work") {
    clothing.pants = true;
    if (jacket) clothing.coat = true;

    if (temperature < -22.0) {
      clothing.longShirt = true;
      clothing.sweater = true;
      clothing.desc =
        "Long sleeves, sweater, coat and hat, heavy " +
      ...
      clothing.sweater = true;
      clothing.desc =
        "Long sleeves, sweater, coat and hat, gloves, " +
      ...
      clothing.sweater = true;
      clothing.desc =
        "Long sleeves, sweater, coat and hat, gloves, " + "earmuffs, and scarf";
    } else if (temperature < 23.0) {
      clothing.longShirt = true;
      clothing.sweater = true;
      clothing.desc =
        "Long sleeves, sweater, coat and hat, gloves, " + "and earmuffs";
    ...
    if (temperature < -22.0) {
      ...
      clothing.sweater = true;
      clothing.desc =
        "Pants, long sleeves, coat and hat, sweater, gloves, " +
      ...
      clothing.sweater = true;
      clothing.desc =
        "Pants, long sleeves, coat and hat, sweater, gloves, " + "shoes";
    } else if (temperature < 41.0) {
      clothing.longShirt = true;
      clothing.sweater = true;
...
function getAttire(combo) {
  ...
  if (combo.longShirt) sugg += combo.longShirt + " shirt";
  else if (combo.shortShirt) sugg += combo.shortShirt + " shirt";
...
$("#activity a").on("shown.bs.tab", function (event) {
  ...
  else $("#jacket").button().prop("disabled", false);
  $("#attire").html(update());
