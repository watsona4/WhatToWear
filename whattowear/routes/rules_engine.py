    ...
    for rs in rules_cfg.get("rule_sets", []):
        ...
        for r in rs.get("rules", []):
            if matches_requires(r.get("when", {}), context):
                ...
                if r.get("note"): notes.append(r["note"])
                if not cont: break
    return {"tags": sorted(tags), "add_items": sorted(add_items), "temp_bias_f": temp_bias, "notes": notes}

class WearHistory:
    def __init__(self, backend: str = "json", json_path: str = "./data/wear_history.json", redis=None):
        self.backend = backend; self.json_path = json_path; self.redis = redis
    ...
    def record(self, outfit_id: str, date: Optional[str] = None):
        ...
        if self.backend == "json":
            ...
            if os.path.exists(self.json_path):
                with open(self.json_path) as f: data = json.load(f)
            data.append(entry)
    ...
    def last_worn_map(self) -> Dict[str, str]:
        if self.backend == "json" and os.path.exists(self.json_path):
            ...
            data = []
        latest = {}
        for e in data: latest[e["outfit"]] = e["date"]
        return latest

def choose_outfit(outfits: Dict[str, Outfit], tags: List[str], context: Dict[str, Any], history: WearHistory, rules_cfg: Dict[str, Any]) -> Outfit:
    ...
    for o in candidates:
        w = 1.0
        for t in o.tags: w *= float(wb.get(t, 1.0))
        weights.append(w)
    import random
